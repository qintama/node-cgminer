 ...
 * @constructor
 */
function CGMinerClient(options, data) {
  if (!_.isObject(options)) {
    options = {};
  }
  this.host = options.host || '127.0.0.1';
  this.port = options.port || 4028;
  createCommandMethod.bind(this)({ name: data, reply: data.toUpperCase()});
  //selectCommand=selectCommand.bind(this)
}
 ...
 */
CGMinerClient.prototype.connect = function (data) {
  ...
  if(data=="stats")
    ...
    return this.pools()  
    .then(function (data) {
      return require('./apis/parser').parse(data.CGMiner);
    })
